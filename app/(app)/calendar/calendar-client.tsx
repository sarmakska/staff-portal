...
const EV: Record<string, Swatch> = {
  ...
  running_late: { grad: "linear-gradient(135deg,#fb923c,#ea580c)", soft: "bg-orange-50 dark:bg-orange-950/40", dot: "bg-orange-400",  ring: "ring-orange-300 dark:ring-orange-700",  badge: "text-orange-600 dark:text-orange-300 bg-orange-100 dark:bg-orange-900/60 border-orange-200 dark:border-orange-800" },
...
const ICONS: Record<string, React.ElementType> = {
  ...
  team: Users, early_leave: Clock, running_late: Clock, visitor: Briefcase,
}
const LEGEND = [
...
export function CalendarClient({
  ...
  /* ══════════════════════════════════════════════════════════ */
  return (
    <div className="bg-muted/30 dark:bg-[#07080c]">
      <style>{CSS}</style>

      {/* ══ TOOLBAR ═══════════════════════════════════════════ */}
      <div className="flex items-center gap-3 px-4 md:px-5 py-3 border-b border-border dark:border-white/[0.08] bg-background/95 dark:bg-[#07080c]/95 backdrop-blur-md sticky top-0 z-20">
        <Link href="/calendar"
        ...
        <div className="hidden md:flex items-center gap-3 mr-2">
          {[
            { val: totalEvs, label: "events", color: "text-primary" },
            { val: leaveEvs, label: "leave", color: "text-rose-500" },
            { val: wfhEvs,   label: "WFH",   color: "text-emerald-500" },
          ].map(s => (
        ...
        <Link href="/calendar/new"
          className="flex items-center gap-1.5 h-8 px-3.5 rounded-xl text-[12px] font-bold bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
        ><Plus className="h-3.5 w-3.5" /> New</Link>
      </div>

      {/* ══ LEGEND — horizontal compact row ═══════════════════ */}
      <div className="hidden md:flex items-center gap-3 px-5 py-2 border-b border-border/60 dark:border-white/[0.05] bg-background/50 dark:bg-[#07080c]/50">
        {LEGEND.map(({ type, label }) => {
          ...
          return (
            <div key={type} className="flex items-center gap-1.5">
              <span className="h-4 w-4 rounded flex items-center justify-center shrink-0" style={{ background: s.grad }}>
                <Icon className="h-2 w-2 text-white" />
              </span>
              <span className="text-[10px] font-medium text-foreground/60">{label}</span>
              {cnt > 0 && <span className={`text-[9px] font-bold px-1 py-0 rounded-full border ${s.badge}`}>{cnt}</span>}
            </div>
          )
        })}
      </div>
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-7 border-b border-border dark:border-white/[0.08] bg-muted/40 dark:bg-[#07080c]">
            {DAYS_SHORT.map((d, i) => (
              <div key={d} className={[
                "py-2 md:py-2.5 text-center font-extrabold uppercase tracking-widest select-none",
                ...
                <span className="hidden md:block text-[10px]">{d}</span>
                <span className="md:hidden text-[9px]">{d[0]}</span>
              </div>
            ))}
          </div>

          {/* Grid */}
          <div className="grid grid-cols-7 bg-background dark:bg-[#07080c]">
            {calendarDays.map((day, idx) => {
              ...
              return (
                <div
                  ...
                  className={[
                    "_sc flex flex-col border-b border-border dark:border-white/[0.07] cursor-pointer select-none",
                    numWeeks > 5 ? "min-h-[44px] md:min-h-[88px]" : "min-h-[48px] md:min-h-[100px]",
                ...
                >
                  {/* Date number */}
                  <div className="flex items-start justify-between px-1 md:px-2.5 pt-1.5 md:pt-2 pb-1 md:pb-1.5">
                    <span className={[
                      "flex items-center justify-center rounded-full font-bold transition-all duration-150",
                      "h-5 w-5 text-[10px] md:h-6 md:w-6 md:text-[11px]",
                    ...
                    {day.events.length > 0 && (
                      <span className="hidden md:block text-[9px] font-bold text-muted-foreground/60 mt-1 tabular-nums">{day.events.length}</span>
                    )}
                  </div>

                  {/* Mobile: coloured dots only */}
                  {day.events.length > 0 && (
                    <div className="flex md:hidden flex-wrap gap-[3px] px-1.5 pb-1.5">
                      {day.events.slice(0, 4).map(e => (
                        <span key={e.id} className={`h-[5px] w-[5px] rounded-full shrink-0 ${sw(e.event_type).dot}`} />
                      ))}
                      {day.events.length > 4 && <span className="h-[5px] w-[5px] rounded-full shrink-0 bg-muted-foreground/30" />}
                    </div>
                  )}

                  {/* Desktop: event chips */}
                  <div className="hidden md:flex flex-col gap-[3px] px-1.5 pb-1.5 flex-1">
                    {day.events.slice(0, 3).map((e, ei) => {
                      ...
                      return (
                        <button
                          ...
                          className="_ch w-full text-left flex items-center gap-1.5 rounded-lg px-2 py-[3px] text-[10px] font-semibold text-white truncate shadow-sm hover:shadow-md hover:scale-[1.015] active:scale-[0.98] transition-all duration-100"
                        >
        ...
        <aside className="_sr hidden lg:flex w-[272px] shrink-0 flex-col gap-3 p-3 border-l border-border dark:border-white/[0.08] bg-background dark:bg-[#0b0c12]">
          ...
          {selDay ? (
            <div className="_pop flex flex-col gap-3">
              <div className="rounded-2xl border border-border/50 dark:border-white/[0.07] overflow-hidden">
                <div className="px-4 py-4 relative" style={{ background: "linear-gradient(135deg, hsl(var(--primary)), hsl(var(--primary)/.65))" }}>
                ...
                {selDay.events.length === 0 ? (
                  ...
                  <div className="divide-y divide-border/40 dark:divide-white/[0.05]">
                    {selDay.events.map(e => {
                      ...
                      const Icon = ICONS[e.event_type] ?? Calendar
                      return (
                        <button key={e.id} onClick={() => setDetEvt({ ...e, date: selDay.date })}
                          className="w-full text-left flex items-center gap-3 px-4 py-3 hover:bg-muted/30 dark:hover:bg-white/[0.04] transition-colors"
                        >
                      ...
                      <div key={day.date} className="h-7 flex items-center justify-center">
                        <button
                          ...
                          className={[
                            ...
                            day.isCurrentMonth ? "text-foreground/75 hover:bg-muted" : "text-muted-foreground/30",
                          ].join(" ")}
                        >
                          {n}
                          {day.events.length > 0 && !day.isToday && day.isCurrentMonth && (
      ...
      {/* ══ THIS WEEK + COMING UP — horizontal strip (desktop) ══ */}
      <div className="hidden lg:grid grid-cols-2 gap-4 px-5 py-4 border-t border-border dark:border-white/[0.08] bg-background dark:bg-[#0b0c12]">
        {/* This Week */}
        <div className="rounded-2xl border border-border dark:border-white/[0.09] overflow-hidden bg-card dark:bg-white/[0.02]">
          <div className="px-4 py-2 border-b border-border/40 dark:border-white/[0.05] bg-muted/30 dark:bg-white/[0.025]">
            <p className="text-[9px] font-black uppercase tracking-[0.15em] text-foreground/65">This Week</p>
          </div>
          <div className="grid grid-cols-7 divide-x divide-border/30 dark:divide-white/[0.04]">
            {thisWeek.map((day: CalendarDay) => {
              const isToday = day.date === todayStr
              const n = parseInt(day.date.split("-")[2])
              return (
                <button key={day.date + "-tw"}
                  onClick={() => setSelDay(prev => prev?.date === day.date ? null : day)}
                  className={`flex flex-col items-center gap-1 px-1 py-3 text-center transition-colors ${isToday ? "bg-primary/[0.06] dark:bg-primary/[0.12]" : "hover:bg-muted/40 dark:hover:bg-white/[0.03]"}`}
                >
                  <span className={`text-[8px] font-black uppercase ${isToday ? "text-primary" : "text-muted-foreground/60"}`}>
                    {fmt(day.date, { weekday: "short" })}
                  </span>
                  <span className={`flex h-7 w-7 items-center justify-center rounded-full text-[12px] font-black ${isToday ? "text-primary-foreground" : "text-foreground/70"}`}
                    style={isToday ? { background: "linear-gradient(135deg, hsl(var(--primary)), hsl(var(--primary)/.7))" } : {}}
                  >{n}</span>
                  <div className="flex flex-col gap-0.5 w-full px-1 mt-1 min-h-[24px]">
                    {day.events.length === 0
                      ? <p className="text-[8px] text-muted-foreground/30 italic">—</p>
                      : day.events.slice(0, 2).map(e => {
                          ...
                          return (
                            <div key={e.id} className="flex items-center gap-1 min-w-0">
                              ...
                              <span className="text-[9px] font-medium text-foreground/70 truncate">{e.title}</span>
                            </div>
                          )
                        })
                    }
                    {day.events.length > 2 && <p className="text-[8px] text-muted-foreground/40">+{day.events.length - 2}</p>}
                  </div>
                </button>
              )
            })}
        ...
        <div className="rounded-2xl border border-border dark:border-white/[0.09] overflow-hidden bg-card dark:bg-white/[0.02]">
          <div className="px-4 py-2 border-b border-border/40 dark:border-white/[0.05] bg-muted/30 dark:bg-white/[0.025]">
            <p className="text-[9px] font-black uppercase tracking-[0.15em] text-foreground/65">Coming Up</p>
          </div>
          {upcoming.length === 0 ? (
            <p className="text-[11px] text-muted-foreground/40 italic px-4 py-4">No upcoming events</p>
          ) : (
            <div className="grid grid-cols-5 divide-x divide-border/30 dark:divide-white/[0.04]">
              {upcoming.slice(0, 5).map(day => (
                <div key={day.date + "-cu"} className="px-2 py-3">
                  <p className="text-[8px] font-bold uppercase tracking-wide text-foreground/50 mb-2 text-center">
                  ...
                  <div className="space-y-1">
                    {day.events.slice(0, 2).map(e => {
                      ...
                      return (
                        <button key={e.id} onClick={() => setDetEvt({ ...e, date: day.date })}
                          className="w-full text-left flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[9px] font-semibold text-white transition-all hover:shadow-md hover:scale-[1.02] active:scale-[0.97]"
                    ...
                    {day.events.length > 2 && <p className="text-[7px] text-muted-foreground/40 text-center">+{day.events.length - 2} more</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {selDay && (
          <div className="_pop rounded-2xl border border-border overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 relative overflow-hidden"
              ...
              <div>
                <p className="text-[9px] font-black uppercase tracking-[0.18em] text-primary-foreground/55">Selected</p>
                <p className="text-[15px] font-black text-primary-foreground leading-none">{fmt(selDay.date, { weekday: "long", day: "numeric", month: "long" })}</p>
              </div>
              <button onClick={() => setSelDay(null)} className="h-7 w-7 flex items-center justify-center rounded-full bg-primary-foreground/20 hover:bg-primary-foreground/30">
                <X className="h-3.5 w-3.5 text-primary-foreground" />
              </button>
            </div>
            <div className="bg-card divide-y divide-border/40">
              {selDay.events.length === 0 ? (
                <p className="text-[11px] text-muted-foreground italic px-4 py-3">No events this day</p>
              ) : selDay.events.map(e => {
                const s = sw(e.event_type)
                const Icon = ICONS[e.event_type] ?? Calendar
                return (
                  <button key={e.id} onClick={() => setDetEvt({ ...e, date: selDay.date })}
                    ...
                    <span className="h-7 w-7 rounded-xl flex items-center justify-center shrink-0" style={{ background: s.grad }}>
